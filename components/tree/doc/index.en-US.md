...
| `[nzHighlightFunc]` | Highlight function against it. The function will receive two arguments, `inputValue` and `option`, finally the value returned will be displayed as `innerHTML`. | `(inputValue?: string, option?: NzTreeNode) => string \| null;` | - |
